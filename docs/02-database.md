...
## orders: услуги наряду с курсами

Миграция `016_service_orders`: `course_id` стал nullable, добавлена колонка `service TEXT`. Ограничение `orders_course_or_service` требует ровно одного из двух — либо курс, либо услуга. В коде `Order.CourseID == uuid.Nil` означает «куплена услуга», её ключ лежит в `Order.Service`.

Сейчас услуга одна: `start` — «Точка перемен», 2 990 ₽. Список в `handlers.services`, цена задаётся там же, а не в базе.
