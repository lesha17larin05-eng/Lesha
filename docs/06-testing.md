...
| `TestLeadsFlow`                        | POST /api/leads: 400 без согласия/имени, 201 при успехе (неизвестный source → `other`, status=new), заявка со страницы «Точка старта» сохраняет `source=start`; /api/admin/leads закрыт для юзера, доступен админу; PATCH статуса работает, невалидный статус → 400. |
