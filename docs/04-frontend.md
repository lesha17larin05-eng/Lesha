...
- `cta` — клик по ссылке на `/start`, `/coaching` или `/courses/zdorovaya-spina`.

Отправка через `navigator.sendBeacon`, с откатом на `fetch(..., {keepalive:true})`. Ничего о посетителе не передаётся.

Цифры видны в `/admin/blog` — колонки «Открыли» (в скобках за 30 дней), «Дочитали» и «На „Точку перемен"» с долей от открытий.
