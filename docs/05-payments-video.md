...
6. Если валидна:
   - находим `order`: перебираем `order_num`, затем `order_id`, каждое пробуем **и как UUID заказа, и как человекочитаемый номер**.

     ⚠️ **Продамус меняет поля местами.** Мы отправляем `order_id=<наш UUID>` и `order_num=<bigint>`, а в вебхуке приходит наоборот: в `order_id` — внутренний номер Продамуса (`48479470`), в `order_num` — наш UUID. Из-за того, что раньше `order_num` парсился только как число, заказ не находился никогда. Единственная покупка через сайт (06.09.2026, «Здоровая спина — Самостоятельный», 3990 ₽) не привязалась, и доступ пришлось выдавать вручную. Починено 17.09.2026, покрыто тестом `TestProdamusWebhookOrderNumCarriesUUID`.

     Остальные 33 вебхука с `no_order` (11.06–12.08.2026, «Здоровая спина в Салюте», 202 150 ₽) — оплаты по ссылкам, которые Алексей выставлял вручную для лагеря. Заказов на сайте для них нет и не должно быть: курс офлайн, доступов в кабинете не требует.
   - Если заказа нет вовсе (оплата по ссылке, выставленной вручную в кабинете Продамуса — там `order_num` содержит имя покупателя): помечаем вебхук `no_order`, доступ **не** выдаём (по названию товара легко перепутать курсы) и шлём Алексею письмо на `LEAD_NOTIFY_EMAIL` со ссылкой на `/admin/grant?email=...`.
...
7. **Возврат денег.** Если `payment_status` содержит `refund` или «возврат» (точное значение у Продамуса может отличаться, поэтому сверяем по вхождению): заказ → `refunded`, доступ, выданный за эту покупку (`granted_by='purchase'`), закрывается, письмо на `LEAD_NOTIFY_EMAIL`. Подаренные (`admin`) и бесплатные (`free`) доступы не трогаются — при возврате за один курс человек не должен потерять остальные. Реализация: `handleRefund` в `payments.go`.
8. Отвечаем 200 всегда (после валидной подписи), иначе будут бесконечные ретраи.
...
## Оплата услуг («Точка перемен»)

Услуга — не курс: доступа к урокам она не даёт, поэтому заказ создаётся с `course_id = NULL` и `service = 'start'` (миграция 016).

1. Человек заполняет имя и почту на `/start` → `POST /api/checkout/service`.
2. Пользователь находится или заводится (`findOrCreateUser`), фиксируется `consent_pd_at`.
3. Создаётся заказ на 2 990 ₽, возвращается ссылка Продамуса. `urlSuccess` → `/oplacheno`.
