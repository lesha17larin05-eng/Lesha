...
6. Если валидна:
   - находим `order`: перебираем `order_num`, затем `order_id`, каждое пробуем **и как UUID заказа, и как человекочитаемый номер**.
     ...
     ⚠️ **Продамус меняет поля местами.** Мы отправляем `order_id=<наш UUID>` и `order_num=<bigint>`, а в вебхуке приходит наоборот: в `order_id` — внутренний номер Продамуса (`48479470`), в `order_num` — наш UUID. Из-за того, что раньше `order_num` парсился только как число, **ни одна оплата с 11.06.2026 не привязалась к заказу** (34 вебхука с `no_order`, 206 140 ₽ мимо статистики), и доступы выдавались вручную. Починено 17.09.2026, покрыто тестом `TestProdamusWebhookOrderNumCarriesUUID`.
