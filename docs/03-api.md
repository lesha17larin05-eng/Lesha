...
| Эндпоинт | Описание |
|---|---|
| `GET /api/admin/segments` | Группы получателей с количеством людей + `daily_limit_hint` (50), `daily_limit_max` (150), `global_daily_cap` (200). |
| `POST /api/admin/campaigns` | `{name, subject, body, segment, daily_limit, pause_sec}` → 201 `{id, total}`. Фиксирует список получателей. Пустые поля → 400 `empty_fields`, неизвестная группа → 400 `bad_segment`. `daily_limit` зажимается в 1…150, `pause_sec` — в 30…3600 (по умолчанию 120). Пишет `audit_log`. |
...
**Отправщик** — фоновая горутина `handlers.RunCampaignWorker`, стартует в `main.go`. Просыпается раз в 10 секунд и берёт рассылку в статусе `sending`, у которой (а) за сутки отправлено меньше `daily_limit` и (б) с последнего письма прошло больше `pause_sec` — то есть интервал между письмами задаётся у каждой рассылки отдельно. Перед каждым письмом проверяет, что согласие ещё в силе (отписавшихся помечает `skipped`). Общий предохранитель `globalDailyCap = 200` писем в сутки по всем рассылкам — чтобы не выбрать суточный лимит ящика, которым уходят письма о доступах. В каждое письмо подставляются персональные ссылка отписки, счётчик открытий и заголовки `List-Unsubscribe`.
...
|-------|------|-----|----------|
| POST | `/api/articles/{slug}/view` | все | Маяк: `{"event":"open"\|"read"\|"cta"}`. Всегда 204 — и на мусор, и на неизвестный slug. Без auth и **без CSRF** (исключение в middleware): запрос уходит через `navigator.sendBeacon`, у которого нет cookie-заголовков. Записи анонимны, подделка даёт только испорченную статистику. Запросы от роботов (по user-agent) не считаются. |
| GET | `/api/admin/article-stats` | админ | Сводка: `opens`, `reads`, `cta`, `opens_30d`, `last_open` по каждой статье, самые читаемые сверху. |

## Подписка на письма с сайта

| Метод | Путь | Кто | Описание |
|-------|------|-----|----------|
| POST | `/api/newsletter` | все | `{"email":"...","consent_pd":true}`. Находит или заводит пользователя (пароль случайный, войти по нему нельзя), фиксирует `consent_pd_at` и шлёт письмо с подписанной ссылкой. Согласие на рассылку ставится **только** после нажатия ссылки — тем же `GET /api/subscribe`, что и в сервисных письмах. Ответ одинаковый для нового и уже существующего адреса, чтобы форма не работала как проверка «есть ли человек в базе». 400 — кривой адрес или нет галочки про ПД. |

## Оплата услуг

| Метод | Путь | Кто | Описание |
|-------|------|-----|----------|
| POST | `/api/checkout/service` | все | `{"service":"start","name":"...","email":"...","consent_pd":true}`. Регистрации не требует: пользователь находится или заводится по почте (пароль случайный). Создаёт заказ без `course_id` и отдаёт `{"url": "<ссылка Продамуса>"}`. 400 — неизвестная услуга, кривой адрес, пустое имя или нет галочки про ПД. |

После оплаты вебхук видит заказ без курса, доступа к курсам **не выдаёт** и вместо этого шлёт два письма: покупателю — что дальше (анкета и время занятия), Алексею на `LEAD_NOTIFY_EMAIL` — кто и что оплатил. Возвраты работают как раньше: `granted_by='purchase'` у услуги нет, поэтому закрывать нечего.
