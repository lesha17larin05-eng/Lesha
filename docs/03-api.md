...
| `POST /api/admin/campaigns` | `{name, subject, body, segment, daily_limit}` → 201 `{id, total}`. Фиксирует список получателей. Пустые поля → 400 `empty_fields`, неизвестная группа → 400 `bad_segment`. `daily_limit` зажимается в 1…150. Пишет `audit_log`. |
...
**Отправщик** — фоновая горутина `handlers.RunCampaignWorker`, стартует в `main.go`. Раз в 45 секунд берёт одну рассылку в статусе `sending`, у которой за сутки отправлено меньше `daily_limit`, и шлёт одно письмо. Перед каждым письмом проверяет, что согласие ещё в силе (отписавшихся помечает `skipped`). Общий предохранитель `globalDailyCap = 200` писем в сутки по всем рассылкам — чтобы не выбрать суточный лимит ящика, которым уходят письма о доступах. В каждое письмо подставляются персональные ссылка отписки, счётчик открытий и заголовки `List-Unsubscribe`.
